...
void STREAMSetFlags(STREAM *S, int Set, int UnSet)
{
    ...
    else RaiseError(0, "", "NULL Stream Passed");
}
...
int STREAMCheckForBytes(STREAM *S)
{
    ...
    if (! S)
    {
        RaiseError(0, "", "NULL Stream Passed");
        return(FALSE);
    }
...
STREAM *STREAMOpen(const char *URL, const char *Config)
{
...
#ifdef USE_MCAST
        else if (strcasecmp(Proto,"mcast")==0) S=STREAMSocketOpen(URL, Config);
#endif
        else S=STREAMFileOpenWithConfig(URL, Config);
        break;
    ...
    case 'b': //b for 'bcast'
        ...
        //bcast, udp, tcp, tls, ssl, etc, etc handled below
        else S=STREAMSocketOpen(URL, Config);
        break;
...
{
    int WaitForBytes=TRUE, val;
    int read_result=STREAM_CLOSED;
    struct timeval tv;
    ...
    if (S)
    {
        ...
        //we set read result == 1 to indicate that we should attempt a read
        //because either WaitForBytes is FALSE or S->Timeout == 0
        //which means we are working in 'blocking' mode, and so always do a read
        else read_result=1;
    }
    else RaiseError(0, "", "NULL Stream Passed");
...
int STREAMWriteString(const char *Buffer, STREAM *S)
{
    ...
    if (StrValid(Buffer)) result=STREAMWriteBytes(S,Buffer,strlen(Buffer));
    return(result);
}
